...
const WorkoutDescription = () => {
  ...
  const RemoveFavorites = async () => {
    try {
      const RemovedData = await axios(
        `${Api}/${Appapi.RemoveFavorite}?email=${userid}&workout_id=${
          HomeCardioData.id
        }&temp=${2}`,
        {
          method: 'GET',
          ...
          <FlatList
            ...
            renderItem={elements => (
              <TouchableOpacity
                style={styles.Days}
                onPress={() => {
                  navigation.navigate('Singleday', {
                    Day: elements.item.id,
                    Id: HomeCardioData.id,
                    DayName: elements.item.days,
                  });
                }}>
                <Text
                  style={{
