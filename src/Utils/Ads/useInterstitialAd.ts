...
// Interface for Interstitial Ad Hook Return
...
// Utility class for managing Interstitial Ads
class InterstitialAdManager {
  // Store ad instances
  private static instances: {[key: string]: InterstitialAd} = {};

  // Track loading promises to prevent multiple simultaneous loads
  private static loadingInstances: {[key: string]: Promise<InterstitialAd> | null} =
    {};

  // Get or create a rewarded ad instance
  static getAdInstance(adUnitId: string): InterstitialAd {
    if (!this.instances[adUnitId]) {
      this.instances[adUnitId] = InterstitialAd.createForAdRequest(adUnitId, {
  ...
  static async preloadAd(adUnitId: string): Promise<InterstitialAd> {
    ...
    const loadPromise = new Promise<InterstitialAd>((resolve, reject) => {
      try {
...
const useInterstitialAd = (): UseInterstitialAdReturn => {
  ...
  const loadAd = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (InterstitialAdManager.isAdLoaded(currentAdUnitId)) {
        setIsAdReady(true);
      ...
      const ad = await InterstitialAdManager.preloadAd(currentAdUnitId);
  ...
  const showAd = useCallback(
    async (onAdClosed?: () => void): Promise<boolean> => {
      ...
      return new Promise(resolve => {
        try {
          const ad = InterstitialAdManager.getAdInstance(getAdUnitId());
